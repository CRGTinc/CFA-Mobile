...
+ (void)__fixLegacyDatabaseLocationIssues
{
         ...
         /*
             314      * There are currently two legacy storage locations:
             315      *   {Lib}/WebKit/LocalStorage which was used for store database (without backup renaming) in ios <5.1 and briefly for ios6, and
             316      *   {Doc}/Backups which was used to store database backups (with backup renaming) with ios5.1+ until Apple started rejecting apps
             317      *      for using the {Doc} folder to save non user generated content.
             318      */
         NSString* appLibraryFolder = [NSSearchPathForDirectoriesInDomains (NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0];
         NSString* appDocumentsFolder = [NSSearchPathForDirectoriesInDomains (NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
